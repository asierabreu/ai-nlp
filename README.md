...
## 🧭 Usage: Build The Knowledge Graph Step By Step

The steps below walk through building a knowledge graph from text using the
`projects/nlp-knowledge-graph` pipeline.
...
Create a raw text file to process.
...
### 5. Run Entity Extraction, Relation Extraction, And Graph Construction
...
Optional: choose a different spaCy model.
...
```bash
head -n 40 data/processed/graph.json
```
...
- A list of entities extracted from text.
- Heuristic relation triples (subject, predicate, object).
- A directed NetworkX knowledge graph.
- A JSON export in `data/processed/graph.json` for downstream tools.
